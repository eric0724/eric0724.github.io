...
# 引入 safe_locate 共用模組（防崩潰 3 輪重試）
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from safe_locate import safe_locate

pyautogui.FAILSAFE = True
pyautogui.PAUSE = 0.2

BASE = os.path.dirname(os.path.abspath(__file__))
TEMPLATE = os.path.join(BASE, "captures", "s1_region.png")
...
def do_locate(template, opt):
    """圖案比對：委派給 safe_locate（3 輪重試 + FAILSAFE 保護）"""
    opt2 = dict(opt)
    opt2["confidence"] = opt2.get("confidence", cfg["confidence"])
    return safe_locate(template, opt2, log=log)
